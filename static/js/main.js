...
if (form) {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (!valid) {
      e.preventDefault(); // Stop submission only if client validation fails
      status.textContent = 'Please fix the highlighted fields.';
      status.classList.add('is-error');
      return;
    }

    // Validation passed -> allow form.submit() to submit natively to Django view
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.project-click-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      const projectId = button.getAttribute('data-id');
      const clickType = button.getAttribute('data-type');

      if (projectId) {
        // Send async request to Django endpoint without delaying navigation
        navigator.sendBeacon(`/track-click/${projectId}/?type=${clickType}`);
      }
    });
  });
});